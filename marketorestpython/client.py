...
class MarketoClient:
    ...
    def _api_call(self, method, endpoint, *args, **kwargs):
        ...
        if self.API_LIMIT and self.API_CALLS_MADE >= self.API_LIMIT:
            raise MarketoException({'message': '# of API Calls exceeded the limit as specified in the Python script: '
        ...
        for i in range(0, 10):
            try:
                ...
                if e.code in ['601', '602']:
                    ...
                    raise MarketoException({'message': e.message, 'code': e.code})
            break
        return result
    ...
    def authenticate(self):
        ...
        data = self._api_call('get', self.host + "/identity/oauth/token", args, mode='accesstoken')
        if data is None:
        ...
        if 'access_token' in data:
            ...
            raise Exception('MarketoClient: unrecognized Authorize response: {}'.format(data))
